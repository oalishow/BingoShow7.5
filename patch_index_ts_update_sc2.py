...
content = content.replace("        function updateCurrentNumberDisplay() {", update_fn + "\n        function updateCurrentNumberDisplay() {")

# Call it inside updateCurrentNumberDisplay
end_fn = """                currentNumberEl.style.visibility = 'visible';
            } else {
                currentNumberEl.style.visibility = 'hidden';
            }
            if (typeof updateSimpleControllerUI === 'function') updateSimpleControllerUI();
        }"""
content = content.replace("""                currentNumberEl.style.visibility = 'visible';
            } else {
                currentNumberEl.style.visibility = 'hidden';
            }
        }""", end_fn)
...
with open('index.tsx', 'w') as f:
    f.write(content)
print("updateSimpleControllerUI added perfectly")
