...
pattern = re.compile(
    r'<div class="flex items-center justify-between border-2 border-slate-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-3 rounded-lg">\s*<label class="text-slate-700 dark:text-slate-300 font-bold" for="card-color">Cor das Cartelas:</label>\s*<input type="color" id="card-color" value="#000000" class="w-12 h-10 p-0 border-0 rounded cursor-pointer">\s*</div>\s*<div class="flex items-center gap-2 border-2 border-slate-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-3 rounded-lg">\s*<input type="checkbox" id="card-reset-series" class="w-5 h-5 rounded cursor-pointer focus:ring-2 focus:ring-sky-500 accent-sky-600 border-gray-300">\s*<label class="text-slate-700 dark:text-slate-300 font-bold cursor-pointer" for="card-reset-series">Zerar numeração de série na geração</label>\s*</div>\s*</div>\s*<div class="flex justify-center gap-4 mb-4">\s*<button id="generate-and-print-cards-btn" class="bg-sky-600 hover:bg-sky-700 text-white font-bold py-3 px-8 rounded-full text-lg w-full">Gerar e Imprimir</button>\s*</div>'
)
...
if pattern.search(content):
    content = pattern.sub(new_modal, content, 1)
    print("Modal patched via regex!")
else:
    print("Modal not found via regex!")
